  ...
  return (
    <section className="w-full bg-transparent px-[20px] mt-[41px] md:mt-[52px] pb-[40px] md:pb-[100px] md:px-[60px] lg:px-[100px] ">
      <div className="max-w-7xl mx-auto text-center">
        {/* Title and Subtitle */}
        <div className="">
          <h1 className="text-[36px] md:text-[56px] font-[700] text-[#2D2D2D] font-['Inter'] leading-normal">
            Your Joints, Our Expertise
          </h1>
          <p className="text-[#2D2D2D] text-[400] text-[16px] md:text-[18px] px-10 md:px-65 leading-[26px] md:leading-[28px] mt-[12px]">
            Minimally invasive solutions from injury to recovery — tailored for athletes and seniors alike.
          </p>
        </div>

        {/* Action Buttons */}
        <div className="flex flex-col sm:flex-row mt-[26px] md:mt-[46px] gap-4 md:gap-8 justify-center items-center max-w-4xl mx-auto">
          <Link
            href="/schedule"
            className="h-14 w-80 bg-[#0D4F7A] text-white px-6 lg:px-8 rounded-[6px] font-[500] font-['Inter'] flex items-center justify-center gap-3 hover:bg-[#1D4ED8] transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0" width="20" height="20" viewBox="0 0 20 20" fill="none">
            ...
            <span className="whitespace-nowrap text-[16px]">Download the Joint Care Playbook</span>
          </Link>
          
          <Link
            href="/consultation"
            className="h-14 w-80 border border-[#0D4F7A] text-[#0D4F7A] px-6 lg:px-8 rounded-[6px] font-[500] font-['Inter'] flex items-center justify-center gap-[10px] hover:bg-[#1E40AF] hover:text-white transition-colors group"
          >

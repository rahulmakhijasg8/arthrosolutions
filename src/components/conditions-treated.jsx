export default function ConditionsTreated({ 
  ...
  return (
    <div className="mt-[32px] md:mt-[60px] " style={{ backgroundColor: '#F4F8FF' }}>
      <h2 className="text-[28px] pt-[32px] md:pt-[64px] md:text-[56px] text-center">
      ...
      <div className="md:hidden flex flex-wrap pb-[32px] px-[20px] gap-x-6 gap-y-3">
        {conditions.map((condition, index) => (
          <div key={index} className="flex bg-white text-[14px] mt-[32px] rounded-[8px] py-4 px-3 items-center">
            <span className="mr-3 text-2xl leading-none" style={{ color: accentColor }}>•</span>
            <span style={{ color: '#64648A', fontWeight: 500 }}>{condition}</span>
          </div>
        ))}
